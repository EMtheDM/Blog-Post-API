...
describe("Back-End Challenge", function () {
  describe("Ping", function () {
    it("Will return the correct body for ping", function (done) {
      request(
    ...
    it("Will return the correct status code for ping where route is correct", function (done) {
      request(
        "http://localhost:3001/api/ping",
        function (error, response, body) {
    ...
    it("Will return the correct status code for ping where route is incorrect", function (done) {
      request(
  ...
  describe("Step 2", function () {
    it("Will return the proper status code for step 2 for the correct route", function (done) {
      request(
        "http://localhost:3001/api/posts/tech",
        function (error, response, body) {
          expect(response.statusCode).to.equal(200);
          done();
        }
    ...
    it("Will return the correct status code for step 2 where route is incorrect", function (done) {
      request(
        "http://localhost:3001/api/post/tech",
        function (error, response, body) {
          expect(response.statusCode).to.equal(404);
          done();
        }
    ...
    it("Will return the correct status code for step 2 when the user uses all three parameters", function (done) {
      request(
        "http://localhost:3001/api/posts/health,tech/likes/desc",
        function (error, response, body) {
          expect(response.statusCode).to.equal(200);
          done();
        }
    ...
    it("Will pass the test if all posts are unique by checking unique ids", function (done) {
      axios
        .get("http://localhost:3001/api/posts/tech,history")
        .then((res) => {
          let post = res.data;
          let postID = [];
          let postObj = {};
    ...
    it("Will pass the test for correctly sorted values", function (done) {
      axios
        .get("http://localhost:3001/api/posts/tech,health/likes/desc")
        .then((res) => {
          let post = res.data;
          let postLikes = [];
          let test = true;
    ...
    it("Will pass the test for correctly sorted values given the default parameters, as if the user did not use them", function (done) {
      axios
        .get("http://localhost:3001/api/posts/tech,health")
        .then((res) => {
          let post = res.data;
          let postID = [];
          let test = true;
